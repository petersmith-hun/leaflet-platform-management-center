...
/**
 * Edit role page.
 * Mapped to /access/roles/edit/:id
 *
 * @param environment APIEnvironment object defining the target API configuration
 */
export default function EditRole(environment: APIEnvironment) {

  const { getRoleByID } = roleService(environment);
  const { t } = useTranslation();
  const { updatePageTitle } = useContext(PageContext);
  const router = useRouter();
  const { isLoading, data, error, mutate } = useSWR(swrStringKey("roles/edit", router.query.id), key => getRoleByID(key.parameter));

  useEffect(() => {
    updatePageTitle(t("page.title.role.edit"));
  }, []);

  return (
    <SWRManagedScreen isLoading={isLoading} error={error}>
      {() => <RoleComposerScreen role={data!} environment={environment} mutate={mutate} />}
    </SWRManagedScreen>
  )
}
