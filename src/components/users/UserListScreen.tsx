...
interface UserListResultProps {
  users: SimplifiedPageModel<UserModel>;
}

const UserListResult = ({ users }: UserListResultProps ): ReactNode => {

  return (
    <ItemListPane pagination={convertPageMeta(users)}>
      <ItemListHeader>
        <ItemListHeaderItem titleKey={"header.user.name-email"} widthClass={"w-4/12"} />
        <ItemListHeaderItem titleKey={"header.user.role"} widthClass={"w-2/12"} />
        <ItemListHeaderItem titleKey={"header.user.status"} widthClass={"w-2/12"} />
        <ItemListHeaderItem titleKey={"header.user.dates"} widthClass={"w-3/12"} />
        <ItemListHeaderItem titleKey={"header.user.operations"} widthClass={"w-1/12"} />
      </ItemListHeader>
      <ItemListBody data={users.content}>
...
export const UserListScreen = ({ environment }: ScreenParameters): ReactNode => {
  ...
  const { isLoading, data, error } = useSWR(swrNumberKey("users/page", query.page ?? 1), key => getAllUsers(key.parameter));
