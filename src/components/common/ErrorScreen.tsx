...
const WarningIcon = ({ colorClass }: WarningIconProps): ReactNode => {
  return (
    <svg
...
/**
 * Renders an error screen based on an API error response.
 *
 * @param response error response, containing the HTTP status
 */
export const ErrorScreen = ({ response }: ErrorParameter): ReactNode => {

  const { t } = useTranslation();

  const warningIcon = ((response?.status ?? 500) < 500)
